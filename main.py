...
class TSMS:
    def __init__(self, time, alpha, beta, A, B, upper_thresholds, lower_thresholds, maximum_tidal_range, salt_scaling_factor, tidal_scaling_factor , df, window=7):
        self.data = df.values[:,:3] # data[:,0] : 담수 유입량(freshwater inflow), data[:,1] : 강수량(rainfall), data[:,2] : 조수 높이 (elevation)
    ...
    #
    def Salt_At_Time(self, t):
        # St = A + B * S(t-1) + S(t)(Q,R,H)
        # flow regime(flow pattern) 은 하구의 위치에 따라 달라지는 상한 및 하한 임계값인 QUT와 QLT에 따라 T일의 총 유입으로 정의된다.
        if (self.flow >= self.upper_thresholds):
            self.s[t] = self.alpha[0] + self.beta[0] * self.salt[self.time-1] + np.finfo(float).eps # 강이 범람한 경우 염분 계산은 큰 의미가 없다 / 입실론은 형식상 표기
        elif (self.flow <= self.lower_thresholds):
            self.s[t] = self.alpha[2] + self.beta[2] * self.salt[self.time-1] + (self.Sr + self.Sh)
        else:
            self.s[t] = self.alpha[1] + self.beta[1] * self.salt[self.time-1] + (self.Sq + self.Sr + self.Sh)
    ...
    def salt_on_freshwater(self, freshwater_at_T):
    ...
    def fresh_water(self):
        ...
        delta_Q = self.salt_step_impulse() # t 시점에서의 담수유입량 이동평균
